...
def build_model(X,y_true,is_train=True,reuse=None,scope='model'):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        #fc2 = tf.contrib.layers.fully_connected(fc2, 100, activation_fn=tf.nn.elu,normalizer_fn = slim.batch_norm)
...
with tf.Session(config=config) as sess:
    ...
    for i in range(0, num_steps):
        # Prepare Data
        _, l = sess.run([optimizer, loss])  # dont use eval()
        # Display logs per step
        l_vs = []; corr_num = 0
        if i % display_step == 0:
            for j in range(VAL_ITER):
                l_v,ypv,ytv = sess.run([loss_v, y_pred_v, y_true_v])
                corr_num += np.sum(ypv == ytv)
                l_vs.append(l_v)
            l_val = np.mean(l_vs)
            acc_v = float(corr_num) / (BATCH_SIZE * VAL_ITER)
            print('Step %i: Minibatch Loss: %f, VAL loss/acc: %f/%f '\
                              % (i, l, l_val, acc_v))
